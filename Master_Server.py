...
class MasterServer(object):
    ...
    def __init__(self, host, port, replica, uploaded_file, fileinfo, file_list):
        self.chunksize=2048												# default chunk size in which file is to be divided
        self.uploaded_file=uploaded_file								# files that are uploaded to chunkservers by client
        self.chunk_servers = {}
        self.file_map = {}
        self.size = 0
        self.num_chunk_servers = 4										# total chunk servers
        self.chunk_servers_info={}
        self.chunk_servers_chunk_count={}
        self.chunk_servers_chunk_count_present={}
        self.replica = replica  										# files and its respective replicas information 
        ...
        self.file_table={}
        self.file_size_info={}											# file + size
        self.active_list=[]
        self.all_file_info = file_list									# list of all files 
        self.filename = ''
        self.host = host
        self.port = port
    ...
    def allocChunks(self):
        ...
        for j in range(0,num_chunks):
            ...
            self.file_map[self.filename].append((j+1,self.active_list[self.active_list.index(minV)]))   # {file name: {chunk no, chunserver no}}
            self.chunk_servers_chunk_count_present[minV] += 1
            self.chunk_servers_chunk_count[minV] += 1
            chunks.append((j+1,self.active_list[self.active_list.index(minV)]))							# {chunk no, chunserver no}
    ...
    def listenToClient(self, client, address,filename,size):
        self.filename=filename											# filename which client wants to upload
        self.size=int(size)												# size of the file

        if self.filename not in self.file_size_info.keys():
            self.file_size_info[filename]=self.size                     # file + size

        # print(self.filename)
        if filename not in self.uploaded_file:
            self.all_file_info[filename]=0								# contains file name & corresponding chunks
    ...
    if os.path.exists("log_file.txt"):        
        with open("log_file.txt","r") as f:
            if os.stat("log_file.txt").st_size != 0:
                ...
                replica = eval(data1)									    # Where replicas of particular chunk is present
                data2 = data2.replace('\'', '')
                fileinfo = eval(data3)										#{(filename, # of chunks)}
                file_list = eval(data4)								     	# all the files list
                uploaded_file = data2.strip('][').split(', ')			    # files uploaded by client
