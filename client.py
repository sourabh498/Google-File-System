...
def connect_to_master_server(getCommand, no_of_arg):
    ...
    if no_of_arg==2:
        ...
        if(decision=="upload"):
            ...
            fileplussize="client"+":upload:"+filename+":"+size                  # to master for uploading: filename + size
            # print(fileplussize)
            s.send(bytes(fileplussize,"utf-8"))                                 # sending files in byte format (can send all types)
            chunks=[]
